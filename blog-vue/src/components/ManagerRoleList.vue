        ...
        methods: {
            ...
            removeRole(index, row){
                this.$confirm('此操将删除该角色, 是否继续?', '提示', {
                    ...
                    let _this= this;
                    let url = this.$store.state.baseUrl + "/role/removeRole?roleId=" + row.id;
                    this.$axios.get(url).then(res =>{
                        if (res.data.code == 200){
            ...
            refreshData(){
                ...
                this.$axios.get(url).then(res =>{
                    if (res.data.code == 200){
                        ...
                        for (let role of res.data.data){
                            let tempUrl = this.$store.state.baseUrl + "/user/listUserByRoleId?roleId=" + role.id;
                            this.$axios.get(tempUrl).then(tempRes =>{
                                if (tempRes.data.code == 200){
