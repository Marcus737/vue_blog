    ...
    export default {
        ...
        methods: {
            login() {
                ...
                let _this = this;
                let loginUrl = _this.$store.state.baseUrl + "/login"
                let loginParams = JSON.stringify({"username": _this.username, "password": _this.pwd});
                _this.$axios.post(loginUrl, loginParams).then(function (loginRes) {
                  if (loginRes.data.code == 200){
                    _this.$store.state.curUser.isLogin = true;
                    _this.$message.success("登录成功");
                    _this.$store.state.curUser.token = loginRes.data.data.tokenHead + " " + loginRes.data.data.token;
                    ...
                    _this.$message.error(loginRes.data.data.msg);
                  }
                })
            },
            createUser(){
