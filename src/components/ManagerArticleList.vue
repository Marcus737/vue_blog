    ...
    export default {
        ...
        methods:{
            ...
            removeArticle(index, row){
                this.$confirm('此操将删除此文章, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    let _this= this;
                    let url = this.$store.state.baseUrl + "/article/removeArticleByArticleId?articleId=" + row.articleId;
                    this.$axios.get(url).then(res =>{
                        if (res.data.code == 200){
            ...
            onSubmit() {
                let _this= this;
                let url = this.$store.state.baseUrl + "/file/uploadArticle";
                ...
                let articleFile = new Blob([this.form.MDValue], {type: 'text/plain'});
            ...
            handleImgAdd(pos, $file){
                ...
                this.$axios({
                    url: this.$store.state.baseUrl + "/file/uploadPicture",
                    method: 'post',
                    data: formdata,
                    headers: { 'Content-Type': 'multipart/form-data'},
                }).then((res) => {
                    if (res.data.code == 200){
                        _this.$refs.md.$img2Url(pos, _this.$store.state.baseDownloadUrl + res.data.msg)
                    }else {
