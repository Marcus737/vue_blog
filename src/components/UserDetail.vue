            ...
            updateUser(){
                ...
                this.$axios.post(ua, p).then(res => {
                  if (res.data.code === 200){
                    _this.imgFile = res.data.msg;
                    _this.$message.success("上传头像成功");
                    let url = _this.$store.state.baseUrl + "/user/updateUser";
                    let params = {};
                    params['userId'] = _this.$store.state.curUser.userId
                    if (_this.username){
                    ...
                    if (_this.imgFile){
                      params['avatar'] = _this.imgFile
                    }
                    if (_this.email){
                      params['email'] = _this.email
                    }
                    this.$axios.post(url, params).then(res1 => {
